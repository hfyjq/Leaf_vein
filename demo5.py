...
def remove_all_paths():
    ...
    删除所有已添加的脚本或应用程序路径
    """
    confirm = input("*确定要删除所有已添加的路径吗？(Y/N): ")
    if confirm.lower() == 'y':
        return []
    else:
        return None
...
def remove_website(websites):
    ...
    website_choice = input("*请输入要删除的网站的编号：")
    try:
...
def open_website(websites):
    ...
    """
    show_websites(websites)
    website_choice = input("*请输入要打开的网站的编号：")
    try:
        website_index = int(website_choice)
